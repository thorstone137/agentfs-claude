import { HookCallbackMatcher, HookEvent, HookJSONOutput, Options, PreToolUseHookInput } from "@anthropic-ai/claude-agent-sdk";
...
async function denyFileSystemToolsHook(
    ...
    return {async: true, hookSpecificOutput: {permissionDecision: "deny", permissionDecisionReason: "You cannot use standard file system tools, you should use the ones from the filesystem MCP server.", hookEventName: "PreToolUse"}}
}

const hooks: Partial<Record<HookEvent, HookCallbackMatcher[]>> = {
    "PreToolUse": [{matcher: "Read|Write|Edit|Glob", hooks: [denyFileSystemToolsHook]} as HookCallbackMatcher],
}
...
export const queryOptions: Options = {
    ...
    allowedTools: ["mcp__filesystem__list_files", "mcp__filesystem__write_file", "mcp__filesystem__edit_file", "mcp__filesystem__read_file", "mcp__filesystem__file_exists"],
